...
# =========================================================================
# ИДЕАЛЬНЫЙ ПАРСЕР СМЕТЫ (Ширина 100%, Жирные заголовки, Удаление нумерации)
# =========================================================================
    ...
    try:
        ...
        def format_table_smart(tbl_element):
            ...
            if len(rows) > 0:
                ...
                # РАДИКАЛЬНАЯ ЧИСТКА: Удаляем вторую строку таблицы (нумерацию)
                if len(rows) > 1:
...
DAMAGE_TEMPLATES = {
    ...
    "[Оптика] Фара (трещина)": "Блок-фара передняя: сквозное разрушение (трещина) рассеивателя.",
}
...
REPAIR_TEMPLATES = {
    ...
    "[Оптика] Замена фары": "Демонтаж, монтаж (замена) блок-фары передней в сборе.",
}

if "damage_text" not in st.session_state: st.session_state.damage_text = DEFAULT_DAMAGE_SUFFIX
if "repair_text" not in st.session_state: st.session_state.repair_text = f"Для восстановления требуется выполнить комплекс слесарно-кузовных, рихтовочных и малярно-окрасочных работ...\n{DEFAULT_REPAIR_SUFFIX}"
